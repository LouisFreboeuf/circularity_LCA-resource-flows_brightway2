...
class BurdenFreeAnalyzer:
    ...
    def __init__(self, project_name, technosphere_db_name, ecoinvent_version, ecoinvent_systemmodel):
...
class CircularityCalculator:
    ...
    """
    def __init__(self, excluded_flows, valuable_compartments, exclude_water=True, technosphere_db_name=None, biosphere_db_name=None):
    ...
    def build_combined_lookup(self):
        ...
        """
        biosphere_db = Database(self.biosphere_db_name)
        techno_to_bio_map = {}
        ...
        for act in db:
            for exc in act.exchanges():
                ...
                #    not (water_content and water_content.get('amount') is not None) and \
                ...
                if not (has_wet_mass or has_water_content or has_heating_value):
                    continue
    ...
    def compute_circularity_efficiency_variables(self, flows_df, save_csv=True, setup_name=None):
        ...
        V = defaultdict(float) # input
        Ri = defaultdict(float) # input recycled (from other economies, from past product systems)
        Rr = defaultdict(float) # output functional tehnosphere (to other economies, to future product systems)
        Er = defaultdict(float) # output functional biosphere (don't damage the environment, to future product systems)
        W = defaultdict(float) # emission flows (damage the environment, not available to future product systems)
        V_mass = 0.0
        Ri_mass = 0.0
        Rr_mass = 0.0
        Er_mass = 0.0
    ...
    def compute_circularity_EMF_indicators(self, flows_df, setup_name=None): # based on the Ellen Mac Arthur Method, simple version available in https://www.ellenmacarthurfoundation.org/material-circularity-indicator
        """
        ...
        results_df, LFI, CFI, detailed_flows_df = self.compute_circularity_efficiency_variables(flows_df, save_csv=False, setup_name=setup_name)
    ...
    def compute_iso59020_recycled_input_rate(self, flows_df, setup_name=None):
        """
    ...
    def compute_iso59020_recycled_output_rate(self, flows_df, setup_name=None):
        """
...
class MultiLCACalculator:
    ...
    def find_activities(self, search_terms, location_codes=None):
        ...
        if not self.location_lookup:
            self.precompute_locations(self.technosphere_db_name)
...
class CircularityDatabaseAnalyzer:
    ...
    """
    def __init__(self, isic_section_map, isic_division_map, location_to_un_group, excluded_flows, valuable_compartments, exclude_water, technosphere_db_name, biosphere_db_name):
        self.EXCLUDED_FLOWS = excluded_flows
        self.VALUABLE_COMPARTMENTS = valuable_compartments
        self.EXCLUDE_WATER = exclude_water
        ...
        self.technosphere_db_name = technosphere_db_name
        self.biosphere_db_name = biosphere_db_name
        self.multi_lca_calculator = MultiLCACalculator(technosphere_db_name)
        self.circularity_calculator = CircularityCalculator(
            excluded_flows,
            valuable_compartments,
    ...
    def compute_circularity_for_all_processes_sequential(self, sample_size=None, save_interval=500):
        """
